...
void __attribute__ ((constructor)) ethInit(void) {
	...
	if (!strcmp(exeName, "et.x86")) {
		...
		if (!getenv("ETH_SELFKILLSPAM"))
			strncpy(skFormat, "^1SUICIDE^3BABY^8!!!",sizeof(skFormat));
		else
			strncpy(skFormat, getenv("ETH_SELFKILLSPAM"),sizeof(skFormat));

		if (!getenv("ETH_KILLSPAM"))
			strncpy(ksFormat, "^2[n] ^7got fucked up!^2[^7Fucks:^3[t]^2]",sizeof(ksFormat));
		else
			strncpy(ksFormat,getenv("ETH_KILLSPAM"),sizeof(ksFormat));

// Set banner/stats format
		if (!getenv("ETH_BANNER"))
			strncpy(bnFormat,"^1              ^1^3ETH^41.3-1 ^7- ^3Edit by Sir3n^1        ^1",sizeof(bnFormat));
		else
